...
| Adapter | Good after hardening | Protocol tests, bounded framing, queued partial writes, heap planner |
| Documentation | Reconciled | Rules, feature matrix, architecture, roadmap, README, and Adapter spec agree |
...
2. TCP input buffering had no line-size limit. Lines are capped at 1 MiB and over-limit connections are closed.
...
- `swift build` with macOS 15.5 SDK: passed after all edits.
- `swift build -c release` with macOS 15.5 SDK: passed after all edits.
...
- The Adapter's 1 MiB limit is intentionally generous. If exposed beyond localhost in the future, add connection-level output backpressure and stricter message-size policy.
